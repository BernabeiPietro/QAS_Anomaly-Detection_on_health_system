...
confusion_field=['a','b','c','d','classified']
...
def confusion_extraction(confusion_section, row):
    ...
    if confusion_section and not (row=="" or row=='   a   b   c   d   <-- classified as'):
        ...
        if words[5]=="d":
            confusion_section=False
        words.append(words[5]+words[6]+words[7])
        words=words[:4]+words[8:]
    return words, confusion_section          
       
if __name__ == '__main__':
    isExist = os.path.exists("multilabel")
    if not isExist:
        os.mkdir("multilabel")
    convert_txt_to_csv('result.txt', 'multilabel/csv_file')
